...
class StatsAnalyzer:
    ...
    def aggregate_data(self, base_dir, target_grids=None):
        ...
        for grid_dir in base_dir.iterdir():
            ...
            # NEW LOGIC: Read flat CSV directly
            stats_file = grid_dir / "sigma_clipped_stats.csv"
            if stats_file.exists():
                try:
                    df = pd.read_csv(stats_file)
                    for _, row in df.iterrows():
                        # Support both old and new column names if possible, but prioritize new
                        # New: grid_id, date, delay, type, clipped_count, mean, median, std
                        entry = {
                            'Grid': row.get('grid_id', grid_id),
                            'Delay': row.get('delay', np.nan),
                            'Type': row.get('type'),
                            'Count': row.get('clipped_count', row.get('count')),
                            'Mean': row.get('mean'),
                            'Std': row.get('std'),
                            'Median': row.get('median', row.get('50%')),
                            'Min': row.get('min', np.nan), # Min/Max might not be in sigma clipped
