...
CSV_PATH = EVO_DIR / "evolution_data.csv"
OUT_DIR = ANALYSIS_DIR / "rain_categories"
...
def main():
    ...
    for cat in categories:
        ...
        # Binned Plot (Smoother than scatter)
        ...
        plt.title(f"Backscatter Evolution: {cat}")
        plt.ylabel("Difference (After - Before) [dB]")
        plt.xlabel("Wait Time (Delay)")
        plt.ylim(-2.0, 2.0) # Fixed scale for comparison
        
        fname = f"evolution_{cat.split(' ')[0].lower()}.png"
        plt.savefig(OUT_DIR / fname)
        plt.close()
        print(f"  Saved plot: {fname}")
        
        # Calculate summary table
        summary = melted.groupby(['Time Bin', 'Type'], observed=False)['Diff (dB)'].agg(['mean', 'count']).unstack()
