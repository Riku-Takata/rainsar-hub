...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(BASE_DIR, "analysis_v5_log.txt"), encoding='utf-8')
    ]
)
    ...
    for entry in verified_entries:
        try:
            res_row = {
                ...
                'road_diff_mean': np.nan, 'road_diff_std': np.nan, 'road_px': 0,
                'paddy_diff_mean': np.nan, 'paddy_diff_std': np.nan, 'paddy_px': 0
            }
            with rasterio.open(entry['path_before']) as src_pre, rasterio.open(entry['path_after']) as src_post:
                ...
                if road_mask_base is not None:
                    try:
                        ...
                        valid = ~np.isnan(diff)
                        valid_diff = diff[valid] # 有効な差分値のみ抽出
                        
                        if len(valid_diff) > 0:
                            # ★ここで平均変化量(dB)を計算
                            res_row['road_diff_mean'] = np.mean(valid_diff)
                            res_row['road_diff_std'] = np.std(valid_diff)
                            res_row['road_px'] = len(valid_diff)
                    except Exception: pass
                ...
                if paddy_mask_base is not None:
                    try:
                        ...
                        if len(valid_diff) > 0:
                            # ★ここで平均変化量(dB)を計算
                            res_row['paddy_diff_mean'] = np.mean(valid_diff)
                            res_row['paddy_diff_std'] = np.std(valid_diff)
                            res_row['paddy_px'] = len(valid_diff)
                    except Exception: pass
    ...
    for i, grid_id in enumerate(grid_dirs):
        ...
        if df_res is not None:
            all_summaries.append({
                'Grid ID': grid_id,
                'Pairs': len(df_res),
                'Road Mean Diff': df_res['road_diff_mean'].mean(),
                'Paddy Mean Diff': df_res['paddy_diff_mean'].mean(),
            })
