...
"""Preprocess downloaded Kanoya Sentinel-1 GRD scenes with SNAP GPT."""
...
DEFAULT_OUTPUT_DIR = ROOT_DIR / "output" / "kanoya_rain_s1" / "processed"
DEFAULT_GPT_EXE = Path(r"C:\Program Files\esa-snap\bin\gpt.exe")

LOGGER = logging.getLogger("kanoya_s1_preprocess")
...
def collect_tasks(rows: list[dict[str, str]], output_dir: Path, pol: str) -> list[dict[str, str]]:
    ...
    for row in rows:
        if "target_stac_id" in row or "pair_stac_id" in row:
            candidates = [
                (row.get("target_stac_id", ""), row.get("target_downloaded", ""), row.get("target_download_path", "")),
                (row.get("pair_stac_id", ""), row.get("pair_downloaded", ""), row.get("pair_download_path", "")),
            ]
        else:
            candidates = [(row.get("stac_id", ""), row.get("downloaded", ""), row.get("download_path", ""))]

        for stac_id, downloaded, zip_path in candidates:
            if downloaded != "True" or not stac_id:
                continue
            scene_stem = normalize_scene_id(stac_id)
            out_tif = output_dir / f"{scene_stem}__{pol}_proc.tif"
    ...
    return sorted(tasks_by_scene.values(), key=lambda task: task["scene_stem"])
    ...
    try:
        for index, task in enumerate(tasks, start=1):
            ...
            if not zip_path.exists():
                ...
                LOGGER.info("[%d/%d] START: %s", index, len(tasks), zip_path.name)
                ok = run_gpt(
            ...
            with result_csv.open("w", newline="", encoding="utf-8-sig") as handle:
